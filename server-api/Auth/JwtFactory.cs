    ...
    public class JwtFactory : IJwtFactory
    {
        ...
        )
        {
            return new TokenValidationParameters{
                ...
                ValidIssuer = jwtAppSettingOptions[nameof(JwtIssuerOptions.Issuer)],//Строка представляющая издателя
        ...
        public static ClaimsIdentity GenerateClaimsIdentity(string userName, string id, IEnumerable<string> roles)
        {
            string authenticationType = "Token";
            var claims = new ClaimsIdentity(
                new GenericIdentity(userName, authenticationType),//имя
                new Claim[] { new Claim(Constants.Strings.JwtClaimIdentifiers.Id, id) }//id
                .Concat(roles.Select(role => new Claim(ClaimTypes.Role, role))).ToArray()//Роли
            );
            return claims;
        }
        ...
        {
            //Собираем утверждения в одну кучу:
